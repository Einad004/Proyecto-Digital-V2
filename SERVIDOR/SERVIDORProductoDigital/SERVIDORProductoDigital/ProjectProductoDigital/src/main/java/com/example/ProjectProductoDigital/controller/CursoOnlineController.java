...
public class CursoOnlineController {
    ...
    public ResponseEntity<List<CursoOnline>> filtrar(
        ...
        return ok(service.filtrar(certificado, precioMax));
    }
    ...
    public ResponseEntity<?> buscarPorParametros(
            ...
            @RequestParam(required = false) Integer idPlataforma) {

        CursoOnline curso = service.buscarPorParametros(nombre, idPlataforma);

        if (curso == null) {
            return ResponseEntity.status(404).body("Curso no encontrado con los parametros enviados");
        }
